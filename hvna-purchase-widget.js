...
class HVNAPurchaseWidget {
    ...
    render() {
        const headerHtml = this.config.showHeader ? `
            <div class="hvna-widget-header">
                ...
                <p>Base Network • Genesis NFT Discounts Available</p>
            </div>
        ` : '';
    ...
    async connectWallet() {
        try {
            ...
            this.showAlert('purchase', 'success', '🎉 Wallet connected successfully!');
    ...
    updateConnectedUI() {
        ...
        if (genesisStatus) {
            if (this.isGenesisHolder) {
                genesisStatus.innerHTML = '<div class="hvna-genesis-badge">✨ Genesis NFT - 30% Discount!</div>';
            } else {
                genesisStatus.innerHTML = '<div style="font-size: 0.9rem; color: #666;">Regular pricing</div>';
            }
        }
    }
