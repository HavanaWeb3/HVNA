...
contract TokenPreSaleAutoForward {
    ...
    modifier validPhase() {
        require(currentPhase != Phase.ENDED, "Sale has ended");
        _;
    }
    ...
    function buyTokens(uint256 tokenAmount) public payable saleIsActive validPhase {
        ...
        // AUTO-FORWARD: Send payment ETH directly to owner
    ...
    function updateTokenPrices(uint256 _genesisCents, uint256 _publicCents) public onlyOwner {
        require(_genesisCents > 0 && _publicCents > 0, "Prices must be greater than 0");
        genesisPriceUSDCents = _genesisCents;
        publicPriceUSDCents = _publicCents;
        emit TokenPricesUpdated(_genesisCents, _publicCents);
    }
    ...
    function setPurchaseLimits(
        ...
        maxPurchaseGenesis = _maxGenesis;
        maxPurchasePublic = _maxPublic;
    }
    ...
    // Emergency withdrawal (if any ETH gets stuck)
    ...
    function emergencyEnd() public onlyOwner {
        ...
        // Withdraw any remaining tokens and ETH
        uint256 remainingTokens = hvnaToken.balanceOf(address(this));
    ...
    // View function to check contract ETH balance
